...
def loadmore():
    ...
    if result:
        ...
        if len(urlQuery) > 0:
            data = {'shortID': urlQuery[0]["shortID"], 'longUrl':  urlQuery[0]["longUrl"],"shortUrl": f'{request.host_url}{urlQuery[0]["shortID"]}','creatTime':  urlQuery[0]["creatTime"]}
            return jsonify({"code": 200, "msg": "URL已存在","data": data })
        shortID = s64(int(time.time()))
        data = {'shortID': shortID, 'longUrl': longURL,"shortUrl":f'{request.host_url}{shortID}','creatTime': time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}
